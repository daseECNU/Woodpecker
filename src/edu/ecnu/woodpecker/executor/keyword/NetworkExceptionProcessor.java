...
public class NetworkExceptionProcessor extends Executor implements Keyword
{
    public NetworkExceptionProcessor()
    {
    ...
    public void handle(String keyword, GrammarType type) throws Exception
    {
        ...
        WpLog.recordLog(LogLevelConstant.INFO, "NET: %s", keyword);
        switch (type)
        {
        case FIRST_GRAMMAR:
    ...
    private void handleFirstGrammar(String keyword)
    {
        ...
        abnormalSimulation.seizeDISK(parts[1], Integer.parseInt(parts[2]), Integer.parseInt(parts[3]),Integer.parseInt(parts[4])); 
    }
